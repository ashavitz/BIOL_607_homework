...
## Eelgrass! {background-image="images/seagrassnet_cover_photo.jpeg"}
:::footer
...
:::::{.columns}
::::{.column width=50%}
1995 ![](images/cape_cod_eg_cover_1995.png){width=60%}  
2023
![](images/cape_cod_eg_cover_2023.png){width=60%}
::::
::::{.column width=50%}
:::{.incremental}
...
- Use of group-averaged predictor: site-level centered mean summer SST
  - Attempt to address unmeasured site-level confounding variables [@suskiewicz_ocean_2024]
  - Not a full Mundlak regression approach - no inclusion of site-level mean SST [@correia_designing_2025]
:::
::::
:::::
...
**With Confounding**  
```{r Confounded DAG Final}
```

## Model System DAG

**Breaking Confounding**  
```{r Final Anomaly DAG}
```
...
:::{.incremental}
- 2 Models: 
  - Percent cover predicted by centered mean summer temperature
  ...
  - Presence - Absence predicted by centered mean summer temperature
...
```{r Visualize Aggregated Percent Cover Model - 2}
```
...
```{r Visualize Aggregated Percent Cover Model - 3}
```
...
:::{.incremental}
- Found *litte to no* evidence that 2003-2021 SST Anomalies are driving NE US eelgrass declines
